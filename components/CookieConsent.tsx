    ...
    <>
      {isPanelOpen ? (
        ...
        <button
          ...
          className="fixed bottom-4 left-4 z-40 inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/70 px-4 py-2 text-xs font-semibold text-gray-300 shadow-lg backdrop-blur-md transition duration-300 hover:border-purple-300/50 hover:text-white focus:outline-none focus:ring-2 focus:ring-purple-300 md:bottom-6 md:left-6"
          aria-label={`Open cookie settings. Current preference: ${
            preference === "accepted" ? "all cookies accepted" : "essential cookies only"
          }`}
        >
          <Settings2 className="h-4 w-4 text-purple-300" aria-hidden="true" />
          Cookie settings
        </button>
      )}
    </>
